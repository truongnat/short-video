...
def get_bgm_file(
    ...
    if bgm_type == "smart":
        ...
        # Build song descriptions for LLM
        song_choices = []
        for f in files:
            name = os.path.basename(f)
            # Default generic matching based on outputxxx name:
            desc = ""
            try:
                ...
                if num_str:
                    num = int(num_str)
                    if num < 10:
                        desc = " (Upbeat, Energetic, Happy, Pop, Dance)"
                    elif num < 20:
                        desc = " (Lofi, Chill, Relaxing, Calm, Acoustic)"
                    elif num < 30:
                        desc = " (Epic, Dramatic, Cinematic, Emotional, Sad)"
            except Exception:
                pass
            song_choices.append(f"- {name}{desc}")

        try:
            from app.services.llm import _generate_response
            prompt = (
                "You are a professional video music director.\n"
                f"Given the video subject: \"{video_subject}\"\n"
                f"and the video script: \"{video_script}\"\n\n"
                "Select the most suitable background music file from the following list of available songs. Choose the one whose style/mood matches the video's content and mood the best:\n"
                f"{chr(10).join(song_choices)}\n\n"
                "Respond ONLY with the exact filename (e.g., \"output001.mp3\") of the chosen song, with no other text, explanation, quotes, or formatting."
            )
            selected_name = _generate_response(prompt).strip()
            # Clean response (remove quotes, brackets, etc.)
            selected_name = selected_name.replace('"', '').replace("'", "").strip()
            # Find the file in list
            for f in files:
                if os.path.basename(f) == selected_name:
                    logger.info(f"Smart BGM selected: {selected_name}")
                    return f
            logger.warning(f"Smart BGM LLM returned invalid filename: {selected_name}, fallback to random.")
        except Exception as e:
            logger.error(f"failed to select smart BGM: {str(e)}, fallback to random.")
...
def generate_video(
    ...
    if params.subtitle_enabled:
        if not params.font_name:
            params.font_name = config.ui.get("font_name", "STHeitiMedium.ttc")
