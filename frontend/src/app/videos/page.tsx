...
export default function Videos() {
  ...
  const regenerateMutation = useMutation({
    mutationFn: (id: string) => api.post(`/videos/${id}/regenerate`).then((res) => res.data),
    onSuccess: (newJob) => {
      queryClient.invalidateQueries({ queryKey: ['videos'] });
      if (newJob?.id) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {videos.map((video) => {
            ...
            >
              ...
              <Link href={`/videos/${video.id}`} className="block relative aspect-[4/5] bg-zinc-900 overflow-hidden flex-shrink-0">
                {thumbnailSrc ? (
                  <img
                    src={thumbnailSrc}
                    alt={video.title}
                ...
                {confirmDeleteId === video.id ? (
                  ...
                  <div className="flex items-center gap-1 mt-auto pt-2 border-t border-zinc-900">
                    ...
                    <button
                      ...
                      title="Render lại"
                    >
                      {regenerateMutation.isPending && regenerateMutation.variables === video.id
