...
export class SettingsService {
  ...
  async getSettings() {
    ...
    const activeProvider = merged.llm_provider || 'groq';
    const providerMeta = LLM_FIELDS.find(
      (field) => field.providerId === activeProvider,
    );
    const providerApiKey = providerMeta
      ? merged[providerMeta.apiKeyField]
      : merged.llm_api_key;
    const llmReady =
      activeProvider === 'gemini'
        ? Boolean(providerApiKey || process.env.GEMINI_API_KEY)
        : Boolean(providerApiKey);

    return {
      ...merged,
      llm_ready: llmReady ? 'true' : 'false',
      llm_provider_active_key: providerMeta?.apiKeyField || 'llm_api_key',
    };
  }
    ...
    try {
      ...
      for (const f of LLM_FIELDS) {
        if (settings[f.apiKeyField] !== undefined) {
          content = this.replaceInSection(
            ...
            'app',
            f.apiKeyField,
            settings[f.apiKeyField],
            false,
            false,
          );
        }
        if (
          settings[f.modelField] !== undefined &&
          settings[f.modelField] !== ''
        ) {
          content = this.replaceInSection(
            content,
            'app',
      ...
      if (settings.twelvelabs_rerank_terms !== undefined) {
        content = this.replaceInSection(
          ...
          settings.twelvelabs_rerank_terms,
          false,
          true,
        );
      }
      ...
      const sectionKeys: Record<
        string,
        Array<{ key: string; isArray: boolean; isBool: boolean }>
      > = {
        azure: [
      ...
      this.logger.log(
        'Successfully synchronized system settings to engine/config.toml',
      );
    } catch (error: any) {
      this.logger.error(
        'Failed to sync settings to engine/config.toml:',
        error,
      );
    }
  }
  ...
  private replaceInSection(
    ...
    if (!replaced) {
      const targetSecIndex = result.findLastIndex((l) =>
        l.match(new RegExp(`^\\[${escapedSection}\\]$`)),
      );
      if (targetSecIndex >= 0) {
